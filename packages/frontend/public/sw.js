...
self.addEventListener('paymentrequest', async e => {
  if (minimalUI) {
    ...
    const dataString = btoa(JSON.stringify(e.methodData[0].data))
    console.log('data string', dataString)
    checkoutURL = `${checkoutURL}?details=${dataString}`
...
self.addEventListener('message', e => {
  ...
  } else if (e.data && e.data.details && e.data.methodName === methodName && e.data.details.code && e.data.details.userMSISDN) {
    try {
...
const sendPaymentRequest = () => {
  ...
  clients.matchAll({
    ...
    for (let client of clientList) {
      client.postMessage(payment_request_event.methodData[0].data)
    }
  })
}
